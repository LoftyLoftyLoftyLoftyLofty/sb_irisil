...
function init()
  ...
  self.openMaterials = config.getParameter("openMaterialSpaces")
  
  if storage.state then
    object.setMaterialSpaces( self.openMaterials )
  else 
    object.setMaterialSpaces( storage.doorStages[#storage.doorStages] )
  end
...
function setupMaterialSpaces()
  ...
  for i, space in ipairs(spaces) do
	...
	if ( tostring(space[1]) == "0" ) then
	  mat = "metamaterial:rail"
	end
