...
function possibleVariationsScriptHook(args)
	...
	--we assume that if there is a kyterran flag, the kyterran race is installed
	if liu_itemExists("flagkyterran") then
		...
		for _, variation in pairs(args) do
			...
			for __, tag in pairs(variation.scriptedTargetTags) do
				...
				--if the tag is species:any, add kyterran
				if tag == "species:any" then
				
					--don't put kyterran in the list twice
					if tableContains( variation.npcSpeciesOptions, "satkyterran" ) == false then
					
						table.insert( variation.npcSpeciesOptions, "satkyterran" )
						--variation.npcSpeciesOptions = { "satkyterran" }
				...
				--if the tag is useVariantIfRaceInstalled:satkyterran, adjust the variant
				if tag == "useVariantIfRaceInstalled:satkyterran" then
				
					--don't put satkyterran in the list twice
					if tableContains( variation.npcSpeciesOptions, "satkyterran" ) == false then
					
						--table.insert( variation.npcSpeciesOptions, "satkyterran" )
						
						--swap out "notInstalled" for "satkyterran" so the spawner has a valid race to use
						variation.npcSpeciesOptions = { "satkyterran" }
...
function finalResultScriptHook(args)
	...
	if args.fnpcType == "lofty_irisil_hylotlpostoffice" then
		...
		--and it's a kyterran
		if args.fnpcSpecies == "satkyterran" then
