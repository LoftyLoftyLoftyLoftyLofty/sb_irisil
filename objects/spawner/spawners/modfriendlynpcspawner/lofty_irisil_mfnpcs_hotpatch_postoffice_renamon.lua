...
function possibleVariationsScriptHook(args)
	...
	if liu_itemExists("flagrenamonlk") then
		...
		for _, variation in pairs(args) do
			...
			for __, tag in pairs(variation.scriptedTargetTags) do
				...
				--if the tag is species:any, add renamon
				if tag == "species:any" then
				
					--don't put skumks in the list twice
					if tableContains( variation.npcSpeciesOptions, "renamonlk" ) == false then
					
						table.insert( variation.npcSpeciesOptions, "renamonlk" )
						--variation.npcSpeciesOptions = { "renamonlk" }
				...
				--if the tag is useVariantIfRaceInstalled:renamonlk, adjust the variant
				if tag == "useVariantIfRaceInstalled:renamonlk" then
				
					--don't put renamon in the list twice
					if tableContains( variation.npcSpeciesOptions, "renamonlk" ) == false then
					
						--table.insert( variation.npcSpeciesOptions, "renamonlk" )
						
						--swap out "notInstalled" for "renamonlk" so the spawner has a valid race to use
						variation.npcSpeciesOptions = { "renamonlk" }
...
function finalResultScriptHook(args)
	...
	if args.fnpcType == "lofty_irisil_hylotlpostoffice" then
		...
		--and it's a renamon
		if args.fnpcSpecies == "renamonlk" then
