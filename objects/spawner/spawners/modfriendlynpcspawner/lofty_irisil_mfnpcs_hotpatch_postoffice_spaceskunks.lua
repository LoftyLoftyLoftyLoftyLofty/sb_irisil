...
function possibleVariationsScriptHook(args)
	...
	if liu_itemExists("flagskumks") then
		...
		for _, variation in pairs(args) do
			...
			for __, tag in pairs(variation.scriptedTargetTags) do
				...
				--if the tag is species:any, add space skunks
				if tag == "species:any" then
				
					--don't put space skunks in the list twice
					if tableContains( variation.npcSpeciesOptions, "skumks" ) == false then
					
						table.insert( variation.npcSpeciesOptions, "skumks" )
						--variation.npcSpeciesOptions = { "skumks" }
				...
				--if the tag is useVariantIfRaceInstalled:skumks, adjust the variant
				if tag == "useVariantIfRaceInstalled:skumks" then
					...
					if tableContains( variation.npcSpeciesOptions, "skumks" ) == false then
						...
						--swap out "notInstalled" for "skumks" so the spawner has a valid race to use
						variation.npcSpeciesOptions = { "skumks" }
...
function finalResultScriptHook(args)
	...
	if args.fnpcType == "lofty_irisil_hylotlpostoffice" then
		...
		--and it's a space skunk
		if args.fnpcSpecies == "skumks" then
