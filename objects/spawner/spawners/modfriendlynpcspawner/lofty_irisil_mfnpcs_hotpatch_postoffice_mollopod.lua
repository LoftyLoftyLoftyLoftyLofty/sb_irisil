...
function possibleVariationsScriptHook(args)
	...
	--we assume that if we have a mollopod flag, the mollopod race is installed
	if liu_itemExists("flagmollopod") then
		...
		for _, variation in pairs(args) do
			...
			for __, tag in pairs(variation.scriptedTargetTags) do
				...
				--if the tag is species:any, add mollopod
				if tag == "species:any" then
				
					--don't put mollopod in the list twice
					if tableContains( variation.npcSpeciesOptions, "mollopod" ) == false then
					
						table.insert( variation.npcSpeciesOptions, "mollopod" )
						--variation.npcSpeciesOptions = { "mollopod" }
				...
				--if the tag is useVariantIfRaceInstalled:mollopod, adjust the variant
				if tag == "useVariantIfRaceInstalled:mollopod" then
					...
					if tableContains( variation.npcSpeciesOptions, "mollopod" ) == false then
						...
						--swap out "notInstalled" for "mollopod" so the spawner has a valid race to use
						variation.npcSpeciesOptions = { "mollopod" }
...
function finalResultScriptHook(args)
	...
	if args.fnpcType == "lofty_irisil_hylotlpostoffice" then
		...
		--and it's a neki
		if args.fnpcSpecies == "mollopod" then
			...
			args.fnpcParameter.scriptConfig.questGenerator.graduation.nextNpcType = 
			{
				...
				{0.5, "lofty_irisil_mollopod_crewmembermechanic"}
			}
