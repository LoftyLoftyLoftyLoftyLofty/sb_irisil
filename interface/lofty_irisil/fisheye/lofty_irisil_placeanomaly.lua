...
desiredDungeon = "lofty_irisil_fisheye"
...
function saveConfiguration()
  ...
  local name = root.assetJson("/system_objects.config:lofty_irisil_fisheye.parameters.displayName")
  local description = root.assetJson("/system_objects.config:lofty_irisil_fisheye.parameters.description")
...
function deployState(system, planets, deployPosition)
  ...
  util.wait(sequence.dispatching, function()
    ...
    View.canvas:drawPoly(circle(orbit, points, View:sToScreen({0, 0})), {72, 127, 57})
  end)
...
function renderOrbits(system, planets, objects, padding, withDeadzones)
  ...
  for _,planet in pairs(planets) do
    ...
    if withDeadzones then
      View.canvas:drawTriangles(wideCircle(orbit, 5 * math.sqrt(orbit), width * View.systemCamera.scale, View:sToScreen({0, 0})), {11, 19, 7, 255})
    end
