...
function deployState(system, planets, deployPosition)
  ...
  local timer = 0
  pane.playSound(self.sounds.launch)
  ...
  if uuid and celestial.objectPosition(uuid) ~= nil then
    ...
    util.wait(2.0, function()
      View.canvas:clear()
      renderOrbits(system, planets, celestial.systemObjects(), self.padding + self.extraPadding, false)
	  --View.canvas:drawPoly(circle(orbit, points, View:sToScreen({0, 0})), {255, 255, 255})
	  View.canvas:drawRect(rect.translate(objectRect, View:sToScreen(deployPosition)), self.activeColor)
    end)
    ...
    util.wait(4.0, function()
	  ...
	  View.canvas:drawRect(rect.translate(objectRect, View:sToScreen(deployPosition)), self.errorColor)
    end)
    pane.dismiss()
    coroutine.yield()
  end
...
function renderSelection(mousePos, selectDistance, validSelection)
  ...
  View.canvas:drawLine({0, mousePos[2]}, {View.windowSize[1], mousePos[2]}, color)
  View.canvas:drawLine({mousePos[1], 0}, {mousePos[1], View.windowSize[2]}, color)
