...
require "/scripts/fishing/lofty_irisil_fancyfishingspawner.lua"
...
function init()
  ...
  message.setHandler("lofty_irisil_setFishingZoneParameters", function(_, _, params)
    --sb.logInfo("received fishing params")
	
    if self.lastKnownFishingZone ~= params.sender then
	  self.lastKnownFishingZone = params.sender
      self.fishingSpawner.setParams(params.args)
      self.fishingSpawner.reset()
	end
  end)

  self.fishingSpawner = FishingSpawner()
  self.fishingSpawner.setLureType( config.getParameter( "fancyLureType" ) )
end
